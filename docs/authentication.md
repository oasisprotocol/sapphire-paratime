...
In Sapphire, you need to consider five types of contract calls:
...
passwords). The MetaMask wallet quickly adopted the standard and it became a
...
Once a user agrees with signing the SIWE login message above, the signature is
verified on the website backend or by a 3rd party [single sign-on] service. This
is done only once—during login. Then, a session token is generated and used for
the rest of the session.

In contrast to transparent EVM chains, **Sapphire simplifies dApp design,
improves trust and extends the usability of SIWE by supporting on-chain SIWE
message parsing and verification**. This unique feature removes the need for
running a separate dApp backend just for SIWE authentication. Let's look at such
authentication flow:

![SIWE authentication flow on Sapphire](diagrams/siwe-sapphire-flow.mmd.svg)

Taking the `MessageBox` contract from [above](#authenticated-view-calls), let's extend it with SIWE:
...
contract MessageBox is SiweAuth {
  ...
  modifier onlyOwner(bytes memory bearer) {
...
sure that the session token for making authenticated calls is valid. If not, the
browser requests a wallet to sign a log-in message and fetches a fresh session
token.
