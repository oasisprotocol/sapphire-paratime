...
Next, let's add the default test accounts after the imports:

![code typescript {5-9}](../examples/hardhat/hardhat.config.ts#config-preamble)
...
3. Add the following tasks to the `hardhat.config.ts`:

<details>
  <summary> hardhat.config.ts </summary>

  ![code typescript {13-66}](../examples/hardhat/hardhat.config.ts)
</details>
