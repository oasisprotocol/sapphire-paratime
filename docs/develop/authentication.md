...
other side—if the SIWE message is forged to match the (exploited) web site
domain, the on-chain [SiweAuth] message validation code will fail and prevent
a user from obtaining a valid token.
...
name when generating the SIWE message. No setters are provided should you wish
to keep the domain immutable. Otherwise, feel free to implement a setter in
your contract with appropriate authentication mechanisms (e.g. `onlyOwner`
modifier). For traceability, we also suggest to **emit an event** in case the
domain is changed since transactions may be encrypted.
...
project. The SIWE authentication is implemented in the backend as a
[Hardhat task], inside [unit tests] and in the frontend inside the
...
encryption for contract calls and transactions, it is not mandatory for SIWE
security, if you trust your Web3 endpoint. The token generation is performed
inside the Sapphire's TEE and the communication with your Web3 endpoint is
secured via HTTPS.
