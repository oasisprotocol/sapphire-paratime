...
The Ethereum virtual machine was designed in a way that you cannot detect 
whether the code is being executed inside the (finalized) transaction or as a
static call (including gas estimation). To overcome this, a developer needs to
store the *proof of finality* in form of a block number when the payment was
received. Then, when obtaining the secret (or performing other actions that 
require the payment), we require a one block delay.

With the following additions, the contract prevents the simulation attack:
