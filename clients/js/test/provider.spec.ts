...
import { Wallet, JsonRpcProvider, BaseContract, JsonRpcSigner, BrowserProvider } from 'ethers';
import { wrapEthereumProvider, NETWORKS, isLegacyProvider, isWrappedEthereumProvider, isWrappedRequestFn } from '@oasisprotocol/sapphire-paratime';
...
describe('Provider Integration Test', () => {
    it('Deploys', async () => {
        ...
        const w = Wallet.fromPhrase('test test test test test test test test test test test junk', v);
        ...
        const storeTxRequest = await w.populateTransaction(await store.populateTransaction(expectedValue));
