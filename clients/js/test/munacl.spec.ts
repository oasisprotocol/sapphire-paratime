...
describe('munacl', () => {
  ...
  it('is_in_scalar_field', () => {
      // L - 2^0
      expect(ed25519_is_valid_scalar(new Uint8Array([
  ...
  describe('ed25519', () => {
    it.skip('Test Cases', () => {
      const cases = [
        ...
        {'message': '72',
         'pub_key': '3d4017c3e843895a92b70aa74d1b7ebc9c982ccf2ec4968cc0cd55f12af4660c',
         'signature': '92a009a9f0d4cab8720e820b5f642540a2b27b5416503f8fb3762223ebdb69da085ac1e43e15996e458f3613d0f11d8c387b2eaeb4302aeeb00d291612bb0c00'},

        {'message': 'af82',
         'pub_key': 'fc51cd8e6218a1a38da47ed00230f0580816ed13ba3303ac5deb911548908025',
      ...
      for( const c of cases ) {
        ...
        const x = ed25519_verify_raw(sig, pbk, msg);
        const outMsg = new Uint8Array(msg.length + 64);
        const y = crypto_sign_open(outMsg, new Uint8Array([...sig, ...msg]), msg.length + 64, pbk);
    ...
    it.skip('ed25519.cr.yp.to sign.output', async () => {
      const output = (await readFile('test/sign.output', 'utf-8')).split("\n");
      let i = 0;
      for( let line of output ) {
        ...
        let [pbk, msg, sig] = line.split(":").map((_) => getBytes(`0x${_}`));
        const outMsg = new Uint8Array(msg.length + 64);
        const y = crypto_sign_open(outMsg, new Uint8Array([...sig, ...msg]), 64 + msg.length, pbk);
        const z = ed25519_verify_raw(sig, pbk, msg);
    ...
    it.skip('Case 1 - small order A', () => {
      const pbk = getBytes("0xc7176a703d4dd84fba3c0b760d10670f2a2053fa2c39ccc64ec7fd7792ac03fa");
      ...
      expect(ed25519_verify_raw(sig, pbk, msg)).toStrictEqual(false); // small order A not rejected
    });

    it.skip('Case 2 - reject small order R', () => {
      const pbk = getBytes("0xf7badec5b8abeaf699583992219b7b223f1df3fbbea919844e3f7c554a43dd43");
      const msg = getBytes("0xaebf3f2601a0c8c5d39cc7d8911642f740b78168218da8471772b35f9d35b9ab");
      const sig = getBytes("0xc7176a703d4dd84fba3c0b760d10670f2a2053fa2c39ccc64ec7fd7792ac03fa8c4bd45aecaca5b24fb97bc10ac27ac8751a7dfe1baff8b953ec9f5833ca260e");
      expect(ed25519_verify_raw(sig, pbk, msg)).toStrictEqual(false); // small order R not rejected
    });
    ...
    it('Case 4 - cofactored verification', () => {
      const pbk = getBytes("0xcdb267ce40c5cd45306fa5d2f29731459387dbf9eb933b7bd5aed9a765b88d4d");
      const msg = getBytes("0xe47d62c63f830dc7a6851a0b1f33ae4bb2f507fb6cffec4011eaccd55b53f56c");
      ...
      expect(ed25519_verify_raw(sig, pbk, msg)).toStrictEqual(false);
    });
  });
});
