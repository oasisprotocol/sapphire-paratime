  ...
  describe('ed25519', () => {
    it('Test Cases', () => {
      ...
      for (const c of cases) {
        ...
        // round-trip point conversion curve25519<->ed25519
        const pbk_curve25519 = curve25519_to_ed25519(pbk);
        const pbk_ed25519 = ed25519_to_curve25519(pbk_curve25519);
        expect(pbk_ed25519).toStrictEqual(pbk);
      }
    });
    ...
    it.skip('ed25519.cr.yp.to sign.output', async () => {
      ...
      for (let line of output) {
        ...
        const x = ed25519_verify_raw_cofactored(sig, pbk, msg);
        const z = ed25519_verify_raw_cofactorless(sig, pbk, msg);
        expect(x).toStrictEqual(z);
        expect(x).toStrictEqual(true);
        expect(z).toStrictEqual(true);
        expect(y).toStrictEqual(msg.length);
      }
