...
library Sapphire {
	 ...
	 */
	function deriveSymmetricKey(bytes32 keyPublic, bytes32 keyPrivate) internal view returns (bytes32) {
	 ...
	 */
	function encrypt(bytes32 key, bytes32 nonce, bytes memory text, bytes memory additionalData) internal view returns (bytes memory result) {
		assembly {
			let p := mload(0x40)
			mstore(p, key)
	 ...
	 */
	function decrypt(bytes32 key, bytes32 nonce, bytes memory ciphertext, bytes memory additionalData) internal view returns (bytes memory result) {
		assembly {
			let p := mload(0x40)
			mstore(p, key)
			mstore(add(p, 0x20), nonce)
			...
			let i := 0
			for {} lt(i, text_len) { i := add(i, 0x20) } {
				mstore(add(add(p, 0x80), i), mload(add(add(ciphertext, 0x20), i)))
			}
			let ad_begin := add(add(p, 0x80), i)
			for { i := 0 } lt(i, ad_len) { i := add(i, 0x20) } {
				mstore(add(ad_begin, i), mload(add(add(additionalData, 0x20), i)))
			...
			mstore(0x40, add(ad_begin, i))
			let out := mload(0x40)
			let success := staticcall(gas(), 0x0100000000000000000000000000000000000004, p, sub(out, p), out, 1)
			if iszero(success) {
				revert(0, 0)
			}
			mstore(out, returndatasize())
