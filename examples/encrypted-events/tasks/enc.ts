...
task('enc', 'Unified emit/listen/decrypt for encrypted events (key|ecdh)')
  ...
  .addOptionalParam(
    'tx',
    'Transaction hash containing the Encrypted event (decrypt)',
  )
  ...
  .setAction(
    async (
      { action, mode, contract, message, key, secret, tx, aadmode, hkdf },
      hre,
    ) => {
      const { ethers } = hre;
      ...
      const getAadBytes = async (
        ...
        if (aadmode === 'sender') {
          if (!senderFromEvent)
            throw new Error(
              'Internal: sender missing from event for aadmode=sender',
            );
      ...
      if (action === 'emit') {
        if (mode === 'key') {
          ...
          let txr;
          if (aadmode === 'sender') {
            txr = await instance.getFunction('emitEncryptedWithAad')(
              keyHex,
              data,
            );
          } else if (aadmode === 'context') {
            txr = await instance.getFunction('emitEncryptedWithContextAad')(
              keyHex,
              data,
            );
          } else {
        ...
        let txr;
        if (aadmode === 'sender') {
          txr = await (instance as any).emitEncryptedECDHWithAad(
            callerPkHex,
            data,
          );
        } else if (aadmode === 'context') {
          txr = await (instance as any).emitEncryptedECDHWithContextAad(
            callerPkHex,
            data,
          );
        } else {
